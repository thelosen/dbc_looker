view: contact_subscriptions {
  ...
  dimension_group: product_subscription_created_eastern_time {
    type: time
    timeframes: [time,date,week,month]
    sql: DATEADD(h,3, ${product_subscription_created_time});;
  }
  ...
  dimension: active_product_subscription {
    type: yesno
    sql: ${recurly_subscription_state} IN ('active','future','past_due') AND ${TABLE}.deleted_at IS NULL AND ${TABLE}.cancelled_at IS NULL AND ${TABLE}.next_renewal_date >= DATEADD(day, -1, GETDATE() )   ;;
  }
