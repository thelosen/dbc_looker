view: combination_2 {
  derived_table: {
    ...
    FROM ((((mysql_heroku_app_db.shop_order_items INNER JOIN mysql_heroku_app_db.shop_order_items AS shop_order_items_1 ON (shop_order_items.order_id = shop_order_items_1.order_id AND shop_order_items.product_id < shop_order_items_1.product_id)) INNER JOIN mysql_heroku_app_db.shop_order_items AS shop_order_items_2 ON shop_order_items_1.order_id = shop_order_items_2.order_id) INNER JOIN mysql_heroku_app_db.product AS product_2 ON shop_order_items_2.product_id = product_2.id) INNER JOIN mysql_heroku_app_db.product AS product_1 ON shop_order_items_1.product_id = product_1.id) INNER JOIN mysql_heroku_app_db.product ON shop_order_items.product_id = product.id
